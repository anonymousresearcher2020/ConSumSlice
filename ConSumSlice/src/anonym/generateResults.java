...
public class generateResults {
    ...
    private void analyzeTestCaseResult(String basePath, String projectName, String bugID, String proPath, String buggyPath, String fixPath, 
		...
		PairList conSumSlicePairList = null;	
		...
		if (newRS != null && oldRs != null) {
			...
			conSumSlicePairList = traceMatcher.matchTraceNodePair(newRS.getRunningTrace(), oldRs.getRunningTrace(), diffMatcher,"conSumSlice");
		...
		rootcauseFinder.setRootCauseBasedOnDefects4J(conSumSlicePairList, diffMatcher, newTrace, oldTrace);
		//////////////////////////
		System.out.println("#################################");
		Simulator simulator = new Simulator(useSliceBreaker, enableRandom, breakLimit);
		simulator.prepare(newTrace, oldTrace, conSumSlicePairList, diffMatcher);//parents in getObservedFault
		...
		if (eraseorDual.equals("S")){
			...
			slicer4J.dualSlicing(basePath,projectName, bugID,tc, true,proPath,observedFaultNode, newTrace, oldTrace, dualPairList, conSumSlicePairList, diffMatcher, oldTraceTime, newTraceTime, codeTime, traceTime,rootcauseFinder.getRealRootCaseList());	
		}
		else if (eraseorDual.equals("E")){
			dualSlicingWithErease Erease = new dualSlicingWithErease();
			Erease.dualSlicing(basePath,projectName, bugID,tc, false, proPath, observedFaultNode, newTrace, oldTrace, dualPairList, conSumSlicePairList, diffMatcher, oldTraceTime, newTraceTime, codeTime, traceTime);
		}
